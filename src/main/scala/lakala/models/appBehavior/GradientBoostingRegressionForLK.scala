package lakala.models.appBehavior
...
/**
  * Created by Administrator on 2017/11/326.
  */
object GradientBoostingRegressionForLK {
  val sparkConf = new SparkConf().setAppName("GradientBoostingRegressionForapp")
  ...
  //训练模型
  def trainModels(): Unit ={
    val data0 = hc.sql(s"select * from lkl_card_score.fqz_community_app_behavior_data where label =0 ").map{
      row =>
        val arr = new ArrayBuffer[Double]()
        //剔除label、order_id字段
        for(i <- 2 until row.size){
    ...
    val data1 = hc.sql(s"select * from lkl_card_score.fqz_community_app_behavior_data where label = 1 ").map{
      row =>
        val arr = new ArrayBuffer[Double]()
        //剔除label、order_id字段
        for(i <- 2 until row.size){
